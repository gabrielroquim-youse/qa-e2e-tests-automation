...
function main(): number {
  ...
  const raw = readFileSync(file, 'utf8').replace(/^\uFEFF/, ''); // strip BOM se houver
  // Remove linhas de comentário do Git (#) e linhas vazias no fim para análise.
  ...
  if (errors.length > 0) {
    ...
    console.log(c('gray', '     feat(quotation): adiciona validação de CPF no endpoint de criação'));
    console.log(c('gray', '     fix(checkout): corrige loop de redirecionamento no pagamento PIX'));
    console.log('');
    return 1;
  }
